...
namespace oft {
    ...
    HandlerDraw::HandlerDraw(bool _flag)
        : ToolsBox(_flag) {}

    HandlerDraw::HandlerDraw(HandlerDraw const& obj)
        : ToolsBox(obj) {}
    ...
    void HandlerDraw::rectangle(cv::Mat& img, cv::Point pt1, cv::Point pt2, const cv::Scalar& color, const int thickness, const int lineType, const int cornerRadius, const int gap) {
        if (this->flag) {
            ...
            // Draw straight lines
            this->line(img, cv::Point(c1.x + cornerRadius, c1.y), cv::Point(c2.x - cornerRadius, c2.y), color, thickness, gap);
            this->line(img, cv::Point(c2.x, cornerRadius + c2.y), cv::Point(c3.x, c3.y - cornerRadius), color, thickness, gap);
            this->line(img, cv::Point(c4.x + cornerRadius, c4.y), cv::Point(c3.x - cornerRadius, c3.y), color, thickness, gap);
            this->line(img, cv::Point(c1.x, cornerRadius + c1.y), cv::Point(c4.x, c4.y - cornerRadius), color, thickness, gap);
